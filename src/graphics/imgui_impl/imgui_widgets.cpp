...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
	...
	PushStyleVar(
		ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y)
...
bool ImGui::BeginComboPreview()
{
	...
	if (window->SkipItems ||
		!window->ClipRect.Overlaps(g.LastItemData.Rect
...
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
	...
	if (fmt_end > fmt_start && fmt_end[-1] == 'f')
	{
		...
		ImGuiContext& g = *GImGui;
		ImFormatString(
			g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), "%.*s%%d%s", (int)(fmt_start - fmt), fmt, fmt_end
		); // Honor leading and trailing decorations, but lose alignment/precision.
		return g.TempBuffer;
	#else
		IM_ASSERT(
			0 && "DragInt(): Invalid format string!"
	...
	if (is_logarithmic)
	{
		...
		else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
		{
			float zero_point_center =
				(-(float)v_min) / ((float)v_max - (float)v_min
								  ); // The zero point in parametric space.  There's an argument we should take the
	...
	if (is_logarithmic)
	{
		...
		else
		{
			...
			if ((v_min * v_max) < 0.0f)   // Range crosses zero, so we have to do this in two parts
			{
				...
				{
					result = (TYPE
					)0.0f; // Special case to make getting exactly zero possible (the epsilon prevents it otherwise)
				}
	...
	switch (data_type)
	{
		...
		case ImGuiDataType_Double:
			...
			return SliderBehaviorT<double, double, double>(
				bb, id, data_type, (double*)p_v, *(const double*)p_min, *(const double*)p_max, format, flags,
				out_grab_bb
			);
		case ImGuiDataType_COUNT: break;
	}
...
)
{
	...
	if (render_cursor || render_selection)
	{
		...
		if (render_selection)
		{
			...
			); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the
			   // transparent one for tests.
	...
	else
	{
		...
		{
			text_size = ImVec2(
				inner_size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * g.FontSize
			); // We don't need width
		}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	float sv_picker_size = ImMax(
		bars_width * 1, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (pressed || (hovered && (flags & ImGuiSelectableFlags_SetNavIdOnHover)))
	{
		...
		{
			SetNavID(
				id, window->DC.NavLayerCurrent, window->DC.NavFocusScopeIdCurrent, WindowRectAbsToRel(window, bb)
			); // (bb == NavRect)
			g.NavDisableHighlight = true;
		}
	}
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
	{
		...
		popup_pos = ImVec2(
			pos.x - 1.0f - IM_FLOOR(style.ItemSpacing.x * 0.5f), pos.y - style.FramePadding.y + window->MenuBarHeight()
		);
		...
		); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window)
{
	...
	IM_ASSERT(
		g.CurrentTabBar != tab_bar
	); // Can't work while the tab bar is active as our tab doesn't have an X offset yet, in theory we could/should test
	   // something like (tab_bar->CurrFrameVisible < g.FrameCount) but we'd need to solve why triggers the commented
	   // early-out assert in BeginTabBarEx() (probably dock node going from implicit to explicit in same frame)

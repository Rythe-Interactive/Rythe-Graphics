...
namespace rythe::rendering::detail
{
    ...
    constexpr const uint64 ryt_mask = ~gl_mask;
...
# define ryt_TESS_CONTROL_SHADER 5
    ...
    const uint64 gl_ryt_vertex_shader = GL_VERTEX_SHADER + (static_cast<uint64>(ryt_VERTEX_SHADER) << 32);
    const uint64 gl_ryt_fragment_shader = GL_FRAGMENT_SHADER + (static_cast<uint64>(ryt_FRAGMENT_SHADER) << 32);
    const uint64 gl_ryt_geometry_shader = GL_GEOMETRY_SHADER + (static_cast<uint64>(ryt_GEOMETRY_SHADER) << 32);
    const uint64 gl_ryt_tess_evaluation_shader = GL_TESS_EVALUATION_SHADER + (static_cast<uint64>(ryt_TESS_EVALUATION_SHADER) << 32);
    const uint64 gl_ryt_tess_control_shader = GL_TESS_CONTROL_SHADER + (static_cast<uint64>(ryt_TESS_CONTROL_SHADER) << 32);

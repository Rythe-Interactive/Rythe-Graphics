	...
	std::pair<bool, std::string> framebuffer::verify() const
	{
		...
		switch (verification)
		{
			...
			case GL_FRAMEBUFFER_UNDEFINED:
				result = {
					false,
					"Id: " + std::to_string(m_id.value) + " Framebuffer set to default but default does not exist."
				};
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
				result = {
					false,
					"Id: " + std::to_string(m_id.value) + " One or more attachments of the framebuffer are incomplete."
				};
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
				result = {
					false,
					"Id: " + std::to_string(m_id.value) + " Framebuffer does not have atleast one image attached to it."
				};
				break;
			case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
				result = {
					false, "Id: " + std::to_string(m_id.value) +
			...
			case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
				result = {
					false, "Id: " + std::to_string(m_id.value) +
							   ...
							   "if the value of GL_TEXTURE_SAMPLES is the not same for all attached textures; If the "
							   "attached images are a mix of renderbuffers and textures, the value of "
							   "GL_RENDERBUFFER_SAMPLES does not match the value of GL_TEXTURE_SAMPLES; or, the value "
							   "of GL_TEXTURE_FIXED_SAMPLE_LOCATIONS is not the same for all attached textures; or, if "
			...
			case GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS:
				result = {
					false,
					"Id: " + std::to_string(m_id.value) +
						" If any framebuffer attachment is layered, and any populated attachment is not layered, or if "
			...
			default:
				result = {
					false,
					"Id: " + std::to_string(m_id.value) + " Unkown framebuffer error: " + std::to_string(verification)
				};
				break;
		}

		if (!result.first) // If the framebuffer isn't complete we might want to know what the state of the framebuffer
						   // is, so we print the attachments.
		{
			glBindFramebuffer(m_target, m_id);
			GLint value;
			glGetFramebufferAttachmentParameteriv(
				m_target, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &value
			);
			switch (value)
			{
				case GL_NONE: log::error("Color attachment is of type GL_NONE"); break;
				case GL_FRAMEBUFFER_DEFAULT: log::error("Color attachment is of type GL_FRAMEBUFFER_DEFAULT"); break;
				case GL_TEXTURE: log::error("Color attachment is of type GL_TEXTURE"); break;
				case GL_RENDERBUFFER: log::error("Color attachment is of type GL_RENDERBUFFER"); break;
				default: log::error("Color attachment is of unknown type: {}", value); break;
			}
			glGetFramebufferAttachmentParameteriv(
				m_target, GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &value
			);
			switch (value)
			{
				case GL_NONE: log::error("Depth attachment is of type GL_NONE"); break;
				case GL_FRAMEBUFFER_DEFAULT: log::error("Depth attachment is of type GL_FRAMEBUFFER_DEFAULT"); break;
				case GL_TEXTURE: log::error("Depth attachment is of type GL_TEXTURE"); break;
				case GL_RENDERBUFFER: log::error("Depth attachment is of type GL_RENDERBUFFER"); break;
				default: log::error("Depth attachment is of unknown type: {}", value); break;
			}
			glGetFramebufferAttachmentParameteriv(
				m_target, GL_STENCIL_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &value
			);
			switch (value)
			{
				case GL_NONE: log::error("Stencil attachment is of type GL_NONE"); break;
				case GL_FRAMEBUFFER_DEFAULT: log::error("Stencil attachment is of type GL_FRAMEBUFFER_DEFAULT"); break;
				case GL_TEXTURE: log::error("Stencil attachment is of type GL_TEXTURE"); break;
				case GL_RENDERBUFFER: log::error("Stencil attachment is of type GL_RENDERBUFFER"); break;
				default: log::error("Stencil attachment is of unknown type: {}", value); break;
			}
			glBindFramebuffer(m_target, 0);
		}
		return result;
#else
		// In release we skip all checks and only check if it's actually valid. For more debug information the engine
		// should be ran in debug mode.
	...
	void framebuffer::attach(attachment att, GLenum attachment)
	{
		...
		if (std::holds_alternative<texture_handle>(att))
		{
			glNamedFramebufferTexture(
				m_id, attachment, std::get<texture_handle>(att).get_texture().textureId, 0
			); // Attach the texture.
		}
		else
		{
			glNamedFramebufferRenderbuffer(
				m_id, attachment, GL_RENDERBUFFER, std::get<renderbuffer>(att).id()

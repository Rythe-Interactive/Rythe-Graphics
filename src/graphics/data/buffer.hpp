...
namespace rythe::rendering
{
	...
	{
	private:
		// Managed resource with the buffer id, also has the responsibility of deleting the buffer after all copies of
		// this buffer have been destroyed.
	...
	public:
		 ...
		 * create a buffer. This means that default initialized buffers are invalid temporary objects until they get
		 * properly initialized.
		 */
		buffer() = default;

		/**@brief Main allocating constructor. This constructor actually creates the GPU side buffer and binds it to
		 * this instance.
		 ...
		 * @param usage Intended memory usage. eg: GL_STATIC_DRAW (Data doesn't change and ised for drawing many times),
		 * GL_DYNAMIC_READ (Changes all the time and is used for reading back to CPU)
		 */
		template <typename T>
		 ...
		 * @param data Data to send to VRAM. This may be a nullptr if you only want to allocate VRAM but keep the memory
		 * empty.
		 * @param usage Intended memory usage. eg: GL_STATIC_DRAW (Data doesn't change and ised for drawing many times),
		 * GL_DYNAMIC_READ (Changes all the time and is used for reading back to CPU)
		 * @note If the buffer is allocated with a nullptr as the data pointer then the memory in VRAM may be garbage
		 * when read. Do not assume null writes to happen.
		 */
		buffer(GLenum target, rsl::size_type size, void* data, GLenum usage);

		/**@brief Main non allocating constructor. This constructor actually creates the GPU side buffer and binds it to
		 * this instance.
		 * @param target The buffer type to create. eg: GL_ARRAY_BUFFER, GL_SHADER_STORAGE_BUFFER
		 * @param usage Intended memory usage. eg: GL_STATIC_DRAW (Data doesn't change and ised for drawing many times),
		 * GL_DYNAMIC_READ (Changes all the time and is used for reading back to CPU)
		 * @note This constructor doesn't actually allocate any VRAM to this buffer yet and requires a bufferData call
		 ...
		 * @note Target must be GL_ATOMIC_COUNTER_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER, GL_UNIFORM_BUFFER or
		...
		/**@brief Send new data to the VRAM of the buffer. This may reallocate the buffer if the size of the new data is
		 * larger than the previously allocated VRAM.
		 ...
		 * @param data Pointer to the data to send. (Should not be a nullptr, if you only want to allocate/reallocate
		 * then use the resize function.)
		 */
		void bufferData(rsl::size_type size, void* data) const;

		/**@brief Send new data to the VRAM of the buffer with a certain offset. This function does not reallocate data
		 * and thus the sum of offset + size cannot be more that the size of the buffer.
